...
BEGIN
    ...
    SELECT
        ...
        pls.total_kills,
        pls.total_deaths,
        calculate_kd_ratio(pls.total_kills, pls.total_deaths),
        pls.total_games,
        pls.games_won,
        calculate_win_percentage(pls.games_won, pls.total_games),
        pls.total_headshots,
        pls.highest_kill_streak
    FROM users u
    JOIN player_lifetime_stats pls ON u.id = pls.user_id
    WHERE u.id = p_user_id;
END;
$$ LANGUAGE plpgsql STABLE;

-- 5. Get Player Weapon Stats
...
BEGIN
    ...
    SELECT
        ...
        pws.kills,
        pws.headshots,
        calculate_accuracy(pws.shots_hit, pws.shots_fired)
    FROM player_weapon_stats pws
    JOIN weapons w ON pws.weapon_id = w.id
    WHERE pws.user_id = p_user_id
...
CREATE OR REPLACE FUNCTION record_kill(
    ...
    p_victim_id UUID,
    p_weapon_id UUID,
    p_is_headshot BOOLEAN DEFAULT false,
    p_is_melee BOOLEAN DEFAULT false,
    p_is_assassination BOOLEAN DEFAULT false,
    p_is_grenade BOOLEAN DEFAULT false
)
RETURNS UUID AS $$
DECLARE
    v_event_id UUID;
...
BEGIN
    ...
    SELECT team_id INTO v_winning_team_id
...
-- 9. Update Kill Streak
CREATE OR REPLACE FUNCTION update_kill_streak(p_game_id UUID, p_user_id UUID, p_increment BOOLEAN)
RETURNS INTEGER AS $$
DECLARE
...
BEGIN
    IF p_increment THEN
        ...
        SELECT id, streak_count INTO v_streak_id, v_streak_count
        FROM kill_streaks
        WHERE game_id = p_game_id AND user_id = p_user_id AND is_active = true;

        IF v_streak_id IS NULL THEN
            INSERT INTO kill_streaks (game_id, user_id, streak_count, is_active)
            VALUES (p_game_id, p_user_id, 1, true)
...
BEGIN
    ...
    SELECT
        ...
        pgs.kills,
        pgs.deaths,
        calculate_kd_ratio(pgs.kills, pgs.deaths),
        pgs.headshots
...
-- 1. Trigger: Update player_game_stats on kill event
CREATE OR REPLACE FUNCTION fn_game_events_stats()
RETURNS TRIGGER AS $$
BEGIN
    -- Update killer stats
    ...
    INSERT INTO player_game_stats (game_id, user_id, deaths)
    VALUES (NEW.game_id, NEW.victim_id, 1)
    ON CONFLICT (game_id, user_id) DO UPDATE SET
        deaths = player_game_stats.deaths + 1;
...
CREATE TRIGGER trg_game_events_stats
AFTER INSERT ON game_events
FOR EACH ROW EXECUTE FUNCTION fn_game_events_stats();

-- 2. Trigger: Update team scores on kill
CREATE OR REPLACE FUNCTION fn_team_score_update()
RETURNS TRIGGER AS $$
DECLARE
    v_killer_team_id UUID;
BEGIN
    -- Get killer's team
    SELECT team_id INTO v_killer_team_id
    FROM game_players
    WHERE game_id = NEW.game_id AND user_id = NEW.killer_id;

    IF v_killer_team_id IS NOT NULL THEN
        UPDATE game_team_scores
        SET score = score + 1
        WHERE game_id = NEW.game_id AND team_id = v_killer_team_id;
    END IF;
...
CREATE TRIGGER trg_team_score_update
AFTER INSERT ON game_events
FOR EACH ROW EXECUTE FUNCTION fn_team_score_update();

-- 3. Trigger: Update weapon stats
CREATE OR REPLACE FUNCTION fn_weapon_stats_update()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO player_weapon_stats (user_id, weapon_id, kills, headshots, shots_hit)
    VALUES (NEW.killer_id, NEW.weapon_id, 1,
            CASE WHEN NEW.is_headshot THEN 1 ELSE 0 END, 1)
...
CREATE TRIGGER trg_weapon_stats_update
AFTER INSERT ON game_events
FOR EACH ROW EXECUTE FUNCTION fn_weapon_stats_update();

-- 4. Trigger: Update lifetime stats when game ends
CREATE OR REPLACE FUNCTION fn_lifetime_stats_on_game_end()
RETURNS TRIGGER AS $$
DECLARE
    v_player RECORD;
    v_playtime INTEGER;
BEGIN
    IF NEW.status = 'completed' AND OLD.status = 'in_progress' THEN
        ...
        FOR v_player IN
            SELECT pgs.user_id, pgs.kills, pgs.deaths, pgs.assists, pgs.headshots,
                   gp.team_id,
        ...
        LOOP
            ...
            SET total_kills = total_kills + v_player.kills,
                total_deaths = total_deaths + v_player.deaths,
                total_assists = total_assists + v_player.assists,
                total_headshots = total_headshots + v_player.headshots,
                total_games = total_games + 1,
                games_won = games_won + v_player.won,
                total_playtime_seconds = total_playtime_seconds + COALESCE(v_playtime, 0)
            WHERE user_id = v_player.user_id;
        END LOOP;

        -- Update highest kill streaks
        UPDATE player_lifetime_stats pls
        SET highest_kill_streak = GREATEST(pls.highest_kill_streak, ks.max_streak)
        FROM (
...
CREATE TRIGGER trg_lifetime_stats_on_game_end
AFTER UPDATE ON games
FOR EACH ROW EXECUTE FUNCTION fn_lifetime_stats_on_game_end();

-- 5. Trigger: Award achievements
CREATE OR REPLACE FUNCTION fn_award_achievements()
RETURNS TRIGGER AS $$
DECLARE
    v_streak_count INTEGER;
    v_achievement RECORD;
BEGIN
    -- Check for headshot achievement
    IF NEW.is_headshot THEN
        FOR v_achievement IN SELECT id FROM achievements WHERE name = 'Headshot' LOOP
            ...
            VALUES (NEW.killer_id, v_achievement.id, NEW.game_id)
            ON CONFLICT DO NOTHING;
        END LOOP;
    END IF;

    -- Check for assassination achievement
    ...
    FROM kill_streaks
    WHERE game_id = NEW.game_id AND user_id = NEW.killer_id AND is_active = true;
...
AFTER INSERT ON game_events
FOR EACH ROW EXECUTE FUNCTION fn_award_achievements();

-- 6. Trigger: Initialize lifetime stats on user creation
CREATE OR REPLACE FUNCTION fn_user_created()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO player_lifetime_stats (user_id)
    VALUES (NEW.id);
...
FOR EACH ROW EXECUTE FUNCTION fn_user_created();

-- 7. Trigger: Initialize team scores when player joins game
CREATE OR REPLACE FUNCTION fn_game_player_joined()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.team_id IS NOT NULL THEN
        INSERT INTO game_team_scores (game_id, team_id, score)
        VALUES (NEW.game_id, NEW.team_id, 0)
...
CREATE TRIGGER trg_game_player_joined
AFTER INSERT ON game_players
FOR EACH ROW EXECUTE FUNCTION fn_game_player_joined();
...
SELECT
    ...
    pls.total_kills,
    pls.total_deaths,
    calculate_kd_ratio(pls.total_kills, pls.total_deaths) as kd_ratio,
    pls.total_games,
    pls.games_won,
    calculate_win_percentage(pls.games_won, pls.total_games) as win_percentage,
    pls.total_headshots,
    pls.highest_kill_streak,
...
LEFT JOIN teams t ON g.winning_team_id = t.id
...
SELECT
    ...
    pgs.kills,
    pgs.deaths,
    calculate_kd_ratio(pgs.kills, pgs.deaths) as kd_ratio,
    pgs.headshots,
    CASE WHEN gp.team_id = g.winning_team_id THEN true ELSE false END as won,
...
FROM weapons w
LEFT JOIN player_weapon_stats pws ON w.id = pws.weapon_id
GROUP BY w.id, w.name, w.category, w.damage_type
...
FROM maps m
LEFT JOIN games g ON m.id = g.map_id
GROUP BY m.id, m.name, m.max_players
ORDER BY times_played DESC;

-- 6. Achievement Progress (per player)
...
LEFT JOIN player_achievements pa ON pa.user_id = u.id AND pa.achievement_id = a.id
...
SELECT
    ...
    (
        ...
        FROM game_team_scores gts
        JOIN teams t ON gts.team_id = t.id
        WHERE gts.game_id = g.id
    ) as team_scores
FROM games g
JOIN maps m ON g.map_id = m.id
JOIN game_types gt ON g.game_type_id = gt.id
...
SELECT
    ...
    ge.game_id,
    ge.created_at,
    ku.gamertag as killer_gamertag,
    ku.id as killer_id,
    kt.name as killer_team_name,
    kt.color as killer_team_color,
    vu.gamertag as victim_gamertag,
    vu.id as victim_id,
    vt.name as victim_team_name,
    vt.color as victim_team_color,
    ...
    ge.is_grenade
FROM game_events ge
JOIN users ku ON ge.killer_id = ku.id
JOIN users vu ON ge.victim_id = vu.id
JOIN weapons w ON ge.weapon_id = w.id
...
LEFT JOIN teams kt ON kgp.team_id = kt.id
LEFT JOIN teams vt ON vgp.team_id = vt.id
ORDER BY ge.created_at DESC;

-- 10. Player Weapon Breakdown
...
FROM users u
JOIN player_weapon_stats pws ON u.id = pws.user_id
JOIN weapons w ON pws.weapon_id = w.id
ORDER BY u.gamertag, pws.kills DESC;
